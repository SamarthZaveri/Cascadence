...
## Docs
- [`PRD.md`](./PRD.md) — what to build, architecture, tech stack, module specs, phased plan
- [`DATA_CONTRACT.md`](./DATA_CONTRACT.md) — authoritative schemas, API/WS shapes, env vars
- [`UPDATES.md`](./UPDATES.md) — living build log; **check this first** when resuming work
...
is stated explicitly rather than hidden — see `PRD.md` §2 and §6 for the full data-source
table and reasoning.
...
git clone <repo>
cd supplyguard
cp .env.example .env   # fill in API keys — see DATA_CONTRACT.md §1 for the full list
...
See `PRD.md` §6 for the full table. Summary: SEC EDGAR (no key, just a compliant
...
- API tests: `httpx.AsyncClient` against the app directly, covering every endpoint in
  `DATA_CONTRACT.md` §4 (happy path, auth failure, validation errors, not-found).
